...
const buildPlaygroundLink = (): HTMLElement => {
	...
	link.setAttribute('href', '#/elements/navigation/z-link')
...
export const zLinkDoc: ComponentDocT = {
	...
	examples: [
		defineMarkupExample({
			...
			markup: `
				<z-text size="md">
				  ...
				  <z-link href="#/elements/actionables/z-button">drop one into an existing page</z-link>
		...
		defineMarkupExample({
			...
			title: 'Block links',
			description:
				'`is-full-width` turns the link into a block-level flex container — useful for list rows or cards where the whole area should be the target.',
			layout: ExampleLayout.fill,
			markup: `
				<wired-column gap="xs" style="width: 340px">
				  <z-link href="#" is-full-width color="neutral">${Icons.mail} Inbox</z-link>
				  <z-link href="#" is-full-width color="neutral">${Icons.calendar} Schedule</z-link>
				  <z-link href="#" is-full-width color="neutral">${Icons.settings} Settings</z-link>
				</wired-column>
			`
		}),
	...
	attributes: [
		...
		{ name: 'is-full-width', type: 'boolean', defaultValue: '—', description: 'Renders as a block-level flex link.' },
	...
	accessibilityNotes: [
		...
		'A disabled (`is-disabled`) link loses its href and therefore leaves the tab order. If the state is temporary, explain why rather than leaving a dead target.'
	],

	related: [
		{ tag: 'z-button', route: '/elements/actionables/z-button', description: 'For actions rather than destinations.' },
		{ tag: 'z-breadcrumbs', route: '/elements/navigation/z-breadcrumbs', description: 'Hierarchical navigation.' },
		{ tag: 'z-nav-menu', route: '/elements/navigation/z-nav-menu', description: 'Primary site navigation.' }
	]
}
