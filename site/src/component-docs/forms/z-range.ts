...
const buildPlaygroundRange = (): HTMLElement => {
	...
	range.setAttribute('does-show-value', '')
	range.setAttribute('value-prefix', '$')
	range.style.width = '100%'

	range.innerHTML = `
		<z-range-handle value="100" accent="dom"></z-range-handle>
		<z-range-handle value="350" accent="dom"></z-range-handle>
	`
...
export const zRangeDoc: ComponentDocT = {
	...
	playground: {
		...
		controlNames: ['label', 'min', 'max', 'step', 'does-show-value', 'value-prefix', 'value-suffix', 'is-disabled'],
	...
	usageGuidance: [
		...
		'Turn on `does-show-value`. A two-handle control without a readout makes the user infer both numbers from thumb positions, which nobody can do accurately.',
	...
	anatomy: [
		{ name: 'header', description: 'Label on the left; with does-show-value, both ends of the span on the right, separated by an en dash.' },
		...
		{ name: 'handles', description: 'Two native range inputs sharing one coordinate system, each with its own accent.' }
	],
	...
	examples: [
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.fill,
			markup: `
				<z-range label="Price" min="0" max="500" step="10" does-show-value value-prefix="$" style="width: 100%">
				  <z-range-handle value="100" accent="dom"></z-range-handle>
				  <z-range-handle value="350" accent="dom"></z-range-handle>
				</z-range>
			`
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.fill,
			markup: `
				<z-range label="Working hours" min="0" max="100" does-show-value value-suffix="%" style="width: 100%">
				  <z-range-handle value="35" min="20" accent="dom"></z-range-handle>
				  <z-range-handle value="70" max="80" accent="dom"></z-range-handle>
				</z-range>
			`
		}),
		...
		defineMarkupExample({
			id: 'accents',
			title: 'Per-handle accents',
			description: 'Each handle carries its own accent, which is useful when the two ends mean different things — a floor and a ceiling, a start and a deadline.',
			layout: ExampleLayout.stack,
			markup: `
				<z-range label="Matched accents" min="0" max="100" does-show-value style="width: 100%">
				  <z-range-handle value="30" accent="dom"></z-range-handle>
				  <z-range-handle value="70" accent="dom"></z-range-handle>
				</z-range>
				<z-range label="Mixed accents" min="0" max="100" does-show-value style="width: 100%">
				  <z-range-handle value="30" accent="dom"></z-range-handle>
				  <z-range-handle value="70" accent="sub"></z-range-handle>
				</z-range>
			`
		}),
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-range label="Continuous" min="0" max="100" does-show-value style="width: 100%">
				  <z-range-handle value="20" accent="dom"></z-range-handle>
				  <z-range-handle value="80" accent="dom"></z-range-handle>
				</z-range>
				<z-range label="Steps of 25" min="0" max="100" step="25" does-show-value style="width: 100%">
				  <z-range-handle value="25" accent="dom"></z-range-handle>
				  <z-range-handle value="75" accent="dom"></z-range-handle>
				</z-range>
			`
		}),
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.fill,
			markup: `
				<z-range label="Disabled" min="0" max="100" does-show-value value-suffix="%" is-disabled style="width: 100%">
		...
		defineInteractiveExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-range id="priceRange" label="Price" min="0" max="1000" step="25" does-show-value value-prefix="$" style="width: 100%">
				  <z-range-handle value="200" accent="dom"></z-range-handle>
				  <z-range-handle value="750" accent="dom"></z-range-handle>
				</z-range>
		...
		defineInteractiveExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-range id="filterRange" label="Price range" min="0" max="400" step="20" does-show-value value-prefix="$" style="width: 100%">
				  <z-range-handle value="60" accent="dom"></z-range-handle>
				  <z-range-handle value="260" accent="dom"></z-range-handle>
				</z-range>
	...
	attributes: [
		...
		{ name: 'does-show-value', type: 'boolean', defaultValue: '—', description: 'Renders both ends of the span in the header.' },
	...
	cssVariables: [
		{ name: '--left-accent', defaultValue: 'var(--primary)', description: 'Accent for the lower handle, derived from that handle’s accent.' },
		{ name: '--right-accent', defaultValue: 'var(--primary)', description: 'Accent for the upper handle, derived from that handle’s accent.' }
	],
	...
	accessibilityNotes: [
		...
		'does-show-value gives the span a visible readout, which matters most for the case where two thumbs sit close together and their positions are ambiguous.'
	],
