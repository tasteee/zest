...
const buildPlaygroundRangeHandle = (): HTMLElement => {
	...
	handle.setAttribute('tone', 'primary')
	return handle
}
...
export const zRangeHandleDoc: ComponentDocT = {
	...
	description:
		'This element renders nothing. It exists so a range can be described in markup rather than configured through a property: put two of them inside a `z-range` and the parent reads their `value`, `min`, `max`, `step`, and `tone` to paint one unified track. The first child is the lower end, the second is the upper. The parent mirrors each live value back onto its handle, so reading `handleElement.value` afterwards always tells the truth.',

	playground: {
		buildElement: buildPlaygroundRangeHandle,
		controlNames: ['value', 'min', 'max', 'step', 'tone'],
	...
	usageGuidance: [
		...
		'Give each handle its own `tone` when the two ends mean different things. Matching tones read as one span; differing tones read as two related-but-distinct values.',
	...
	anatomy: [
		...
		{ name: 'tone', description: 'The accent this end’s thumb is painted in.' },
	...
	examples: [
		...
		defineMarkupExample({
			...
			markup: `
				<z-range label="Threshold band" min="0" max="100" show-value value-suffix="%" style="width: 100%">
				  <z-range-handle value="40" min="25" tone="primary"></z-range-handle>
				  <z-range-handle value="70" max="85" tone="primary"></z-range-handle>
				</z-range>
			`
		}),
		...
		defineMarkupExample({
			id: 'tones',
			title: 'Tones',
			description: 'Each handle paints its own thumb. Differing tones are worth it when the two ends carry different meaning.',
			layout: ExampleLayout.fill,
			markup: `
				<z-range label="Soft and hard limit" min="0" max="100" show-value style="width: 100%">
				  <z-range-handle value="45" tone="primary"></z-range-handle>
				  <z-range-handle value="80" tone="secondary"></z-range-handle>
				</z-range>
			`
		}),
		...
		defineMarkupExample({
			...
			markup: `
				<z-range label="Price range" min="0" max="500" step="10" show-value value-prefix="$" style="width: 100%">
				  <z-range-handle value="120" label="Minimum price" tone="primary"></z-range-handle>
				  <z-range-handle value="380" label="Maximum price" tone="primary"></z-range-handle>
				</z-range>
			`
		}),
		...
		defineMarkupExample({
			...
			markup: `
				<z-range label="Mixed precision" min="0" max="100" show-value style="width: 100%">
				  <z-range-handle value="22" tone="primary"></z-range-handle>
				  <z-range-handle value="75" step="5" tone="primary"></z-range-handle>
				</z-range>
			`
		}),
		...
		defineInteractiveExample({
			...
			markup: `
				<z-range id="readbackRange" label="Span" min="0" max="100" show-value style="width: 100%">
				  <z-range-handle class="readbackHandle" value="20" tone="primary"></z-range-handle>
				  <z-range-handle class="readbackHandle" value="60" tone="primary"></z-range-handle>
				</z-range>
	...
	attributes: [
		...
		{ name: 'tone', type: 'neutral | primary | secondary', defaultValue: 'neutral', description: 'Accent for this handle’s thumb.' },
		{
			name: 'label',
			type: 'string',
