...
const buildPlaygroundSlider = (): HTMLElement => {
	...
	slider.setAttribute('tone', 'primary')
	return slider
}
...
export const zSliderDoc: ComponentDocT = {
	...
	description:
		'Built on a native `input[type=range]`, then restyled completely: hairline track, accent-filled progress, solid accent thumb. Building on the native control is what buys full keyboard support, correct touch behaviour, and screen-reader announcement for free. An optional header carries the `label` on the left and — with `show-value` — the live number on the right as an accent pill, with `value-prefix` and `value-suffix` for units.',

	playground: {
		buildElement: buildPlaygroundSlider,
		controlNames: ['label', 'min', 'max', 'step', 'tone', 'show-value', 'value-prefix', 'value-suffix', 'is-disabled'],
		slotLabel: 'Volume'
	},
	...
	usageGuidance: [
		...
		'Turn on `show-value` unless the effect is immediately visible elsewhere. A slider with no readout asks the user to guess where they landed.',
	...
	examples: [
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.fill,
			markup: `
				<z-slider label="Volume" value="60" show-value value-suffix="%" tone="primary" style="width: 100%"></z-slider>
			`
		}),
		...
		defineMarkupExample({
			...
			markup: `
				<z-slider label="Budget" min="0" max="500" step="25" value="150" show-value value-prefix="$" tone="primary" style="width: 100%"></z-slider>
				<z-slider label="Retention" min="1" max="90" value="30" show-value value-suffix=" days" tone="primary" style="width: 100%"></z-slider>
				<z-slider label="Opacity" min="0" max="1" step="0.05" value="0.75" show-value tone="primary" style="width: 100%"></z-slider>
			`
		}),
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-slider label="Neutral" tone="neutral" value="35" show-value style="width: 100%"></z-slider>
				<z-slider label="Primary" tone="primary" value="55" show-value style="width: 100%"></z-slider>
				<z-slider label="Secondary" tone="secondary" value="75" show-value style="width: 100%"></z-slider>
			`
		}),
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-slider label="Continuous" value="47" show-value tone="primary" style="width: 100%"></z-slider>
				<z-slider label="Steps of 10" step="10" value="50" show-value tone="primary" style="width: 100%"></z-slider>
			`
		}),
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.fill,
			markup: `
				<z-slider label="Disabled" value="65" show-value value-suffix="%" is-disabled style="width: 100%"></z-slider>
			`
		}),
		...
		defineInteractiveExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-slider id="qualitySlider" label="Export quality" min="0" max="100" step="5" value="70" show-value value-suffix="%" tone="primary" style="width: 100%"></z-slider>
		...
		defineInteractiveExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-slider id="radiusSlider" label="Corner radius" min="0" max="48" value="12" show-value value-suffix="px" tone="primary" style="width: 100%"></z-slider>
	...
	attributes: [
		...
		{ name: 'show-value', type: 'boolean', defaultValue: '—', description: 'Renders the live value as a pill on the right of the header.' },
		...
		{ name: 'tone', type: 'neutral | primary | secondary', defaultValue: 'neutral', description: 'Accent for the fill, the thumb, and the value pill.' },
	...
	accessibilityNotes: [
		...
		'The value is announced by the native control as it changes, so show-value is a visual convenience rather than an accessibility requirement — but it helps everyone.',
