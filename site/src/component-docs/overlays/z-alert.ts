...
export const zAlertDoc: ComponentDocT = {
	...
	playground: {
		...
		controlNames: ['accent', 'heading', 'is-dismissable'],
	...
	anatomy: [
		{ name: 'icon', description: 'The status glyph, chosen by accent. Decorative — the accent is also carried by the role and the copy.' },
	...
	examples: [
		defineMarkupExample({
			...
			markup: `
				<z-alert accent="neutral" heading="Draft saved">This project has not been shared with anyone yet.</z-alert>
				<z-alert accent="dom" heading="Scheduled maintenance">The API will be read-only on Sunday from 02:00 to 04:00 UTC.</z-alert>
				<z-alert accent="success" heading="Domain verified">zest.app is now serving over HTTPS.</z-alert>
				<z-alert accent="warning" heading="Approaching your limit">You have used 92% of this month’s allowance.</z-alert>
				<z-alert accent="error" heading="Payment failed">Update your card to keep your projects running.</z-alert>
			`
		}),
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-alert accent="dom">Two-factor authentication is now required for every member of this workspace.</z-alert>
				<z-alert accent="warning">This API version is deprecated and will stop responding in March.</z-alert>
			`
		}),
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-alert accent="error" heading="Payment failed">
		...
		defineMarkupExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-alert accent="dom" heading="New in this release" is-dismissable>
		...
		defineInteractiveExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-alert id="tipAlert" accent="dom" heading="Keyboard shortcuts" is-dismissable>
				  Press ⌘K anywhere to open the command palette.
				</z-alert>
				<z-text size="sm" color="muted" id="tipStatus">Alert is showing.</z-text>
				<z-button id="restoreTip" size="sm" kind="outline">Bring it back</z-button>
			`,
			script: `
		...
		defineInteractiveExample({
			...
			markup: `
				...
				<z-button id="saveWorkspace" kind="solid" accent="dom">Save</z-button>
			`,
			script: `
	...
	attributes: [
		{ name: 'accent', type: 'neutral | dom | success | warning | error', defaultValue: 'neutral', description: 'The condition being reported. Picks the tint and the icon.' },
