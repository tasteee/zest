...
const buildPlaygroundCard = (): HTMLElement => {
	...
	card.setAttribute('heading', 'Card title')
	card.setAttribute('description', 'Supporting copy inside the card.')
	card.innerHTML = `
		<z-text size="sm">The slot picks up below the header.</z-text>
	`
	return card
}
...
export const zCardDoc: ComponentDocT = {
	...
	playground: {
		...
		controlNames: ['heading', 'description', 'is-reactive'],
		slotLabel: 'Card body'
	},

	usageGuidance: [
		'Use a card when content is a discrete unit you could reorder or remove without disturbing what is around it.',
		'Prefer `heading` over slotting your own z-heading. That is what keeps every card title the same size as every other card title — and the same as a sheet or dialog title.',
		'Slot a z-heading instead when the title genuinely needs a different heading level or a different size. The attribute renders an h3 and does not negotiate.',
		...
		'A card inside a card almost always means the inner one should be a `z-separator` and some spacing instead.',
		'Reach for `z-surface` when you need a accent, a variant, or a specific elevation level; `z-card` deliberately has none of those.'
	],

	anatomy: [
		{ name: 'header', description: 'The optional heading and description, rendered only when one of them is set.' },
		{ name: 'default slot', description: 'The card contents, below the header.' },
	...
	examples: [
		...
		defineMarkupExample({
			...
			description: 'A `z-separator` splits body from footer without introducing a second nested surface.',
			layout: ExampleLayout.fill,
			markup: `
				<z-card style="max-width: 24rem">
				  <wired-column gap="md">
	...
	attributes: [
		{
			...
			type: 'string',
			defaultValue: '—',
			description: 'Card title, rendered as an h3 in the shared surface-title style. Omit it and no header is rendered at all.'
		},
		{
			name: 'description',
			type: 'string',
			defaultValue: '—',
			description: 'A line of supporting copy under the heading. Works with or without one.'
		},
		{
			name: 'is-reactive',
			type: 'boolean',
	...
	related: [
		{ tag: 'z-surface', route: '/elements/structure/z-surface', description: 'Neutral surface treatments and outlined panels.' },
		{ tag: 'z-bento-item', route: '/elements/structure/z-bento-item', description: 'A richer card cell for feature grids.' },
		{ tag: 'z-list-row', route: '/elements/data-display/z-list-row', description: 'For rows rather than blocks.' },
		{ tag: 'z-separator', route: '/elements/structure/z-separator', description: 'Dividing a card internally.' }
	]
}
