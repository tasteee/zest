...
const buildPlaygroundToggle = (): HTMLElement => {
	const toggle = document.createElement('z-toggle-button')
	toggle.setAttribute('accent', 'dom')
	toggle.innerHTML = `${Icons.bold} Bold`
	return toggle
}

export const zToggleButtonDoc: ComponentDocT = {
	tag: 'z-toggle-button',
	...
	usageGuidance: [
		...
		'For a set of mutually exclusive options, use `z-toggle-button-group` in single mode rather than wiring several toggles together by hand.'
	],
	...
	examples: [
		defineMarkupExample({
			...
			markup: `
				<z-toggle-button>${Icons.bold} Bold</z-toggle-button>
				<z-toggle-button is-pressed>${Icons.bold} Bold</z-toggle-button>
			`
		}),
		...
		defineMarkupExample({
			...
			markup: `
				<z-toggle-button accent="neutral" is-pressed>Neutral</z-toggle-button>
				<z-toggle-button accent="dom" is-pressed>Primary</z-toggle-button>
				<z-toggle-button accent="sub" is-pressed>Secondary</z-toggle-button>
			`
		}),
		...
		defineMarkupExample({
			...
			markup: `
				<z-toggle-button kind="outline">Outline</z-toggle-button>
				<z-toggle-button kind="outline" is-pressed>Outline</z-toggle-button>
				<z-toggle-button kind="ghost">Ghost</z-toggle-button>
				<z-toggle-button kind="ghost" is-pressed>Ghost</z-toggle-button>
			`
		}),
		...
		defineMarkupExample({
			...
			markup: `
				<z-toggle-button size="sm" is-pressed>Small</z-toggle-button>
				<z-toggle-button size="md" is-pressed>Medium</z-toggle-button>
				<z-toggle-button size="lg" is-pressed>Large</z-toggle-button>
			`
		}),
		...
		defineMarkupExample({
			...
			markup: `
				<z-toggle-button is-icon aria-label="Bold" is-pressed>${Icons.bold}</z-toggle-button>
				<z-toggle-button is-icon aria-label="Italic">${Icons.italic}</z-toggle-button>
				<z-toggle-button is-icon aria-label="Underline">${Icons.underline}</z-toggle-button>
			`
		}),
		...
		defineMarkupExample({
			...
			markup: `
				<z-toggle-button is-disabled>Off and disabled</z-toggle-button>
				<z-toggle-button is-pressed is-disabled>On and disabled</z-toggle-button>
			`
		}),
		...
		defineInteractiveExample({
			...
			markup: `
				<z-toggle-button id="muteToggle" accent="dom">Mute notifications</z-toggle-button>
		...
		defineInteractiveExample({
			...
			markup: `
				<z-toolbar id="formatBar">
				  <z-toggle-button is-icon kind="ghost" aria-label="Bold" data-format="bold">${Icons.bold}</z-toggle-button>
				  <z-toggle-button is-icon kind="ghost" aria-label="Italic" data-format="italic">${Icons.italic}</z-toggle-button>
				  <z-toggle-button is-icon kind="ghost" aria-label="Underline" data-format="underline">${Icons.underline}</z-toggle-button>
				</z-toolbar>
			...
			wire: (root) => {
				...
				const describeActiveFormats = (): string => {
					// Reads the live `.isPressed` property rather than the
					// `is-pressed` attribute: Atomico reflects props to attributes
					// in a microtask after `press` fires, so hasAttribute() here
					// would still see the toggle's state from before this click.
					const toggles = [...formatBar.querySelectorAll('z-toggle-button')] as (HTMLElement & { isPressed?: boolean })[]
					const pressedToggles = toggles.filter((toggle) => toggle.isPressed)
					const formatNames = pressedToggles.map((toggle) => toggle.dataset.format)
	...
	attributes: [
		...
		{ name: 'size', type: 'xs | sm | md | lg', defaultValue: 'md', description: 'Control density.' },
	...
	related: [
		{ tag: 'z-toggle-button-group', route: '/elements/actionables/z-toggle-button-group', description: 'Coordinate several toggles as one control.' },
		{ tag: 'z-switch', route: '/elements/actionables/z-switch', description: 'The settings-row equivalent.' },
		{ tag: 'z-button', route: '/elements/actionables/z-button', description: 'For actions with no persistent state.' }
	]
}
