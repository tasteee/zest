...
export const zToolbarDoc: ComponentDocT = {
	...
	usageGuidance: [
		...
		'Divide unrelated clusters with `z-separator vertical`, or name them with `z-toolbar-group` when the grouping needs to be announced rather than just seen.',
	...
	examples: [
		...
		defineMarkupExample({
			...
			markup: `
				<z-toolbar>
				  ...
				  <z-separator vertical></z-separator>
				  <z-toggle-button is-icon kind="ghost" aria-label="Bold">${Icons.bold}</z-toggle-button>
				  <z-toggle-button is-icon kind="ghost" aria-label="Italic">${Icons.italic}</z-toggle-button>
				  <z-toggle-button is-icon kind="ghost" aria-label="Underline">${Icons.underline}</z-toggle-button>
				  <z-separator vertical></z-separator>
		...
		defineMarkupExample({
			...
			markup: `
				<z-toolbar>
				  <z-toggle-button-group accent="dom" kind="ghost">
				    <z-toggle-button-group-item value="left" is-icon aria-label="Align left" is-pressed>${Icons.alignLeft}</z-toggle-button-group-item>
				    <z-toggle-button-group-item value="center" is-icon aria-label="Align center">${Icons.alignCenter}</z-toggle-button-group-item>
				    <z-toggle-button-group-item value="right" is-icon aria-label="Align right">${Icons.alignRight}</z-toggle-button-group-item>
	...
	related: [
		{ tag: 'z-toolbar-group', route: '/elements/actionables/z-toolbar-group', description: 'A labelled cluster inside a toolbar.' },
		{ tag: 'z-button', route: '/elements/actionables/z-button', description: 'The usual toolbar control.' },
		{ tag: 'z-toggle-button', route: '/elements/actionables/z-toggle-button', description: 'Stateful toolbar controls.' },
		{ tag: 'z-separator', route: '/elements/structure/z-separator', description: 'Draws a vertical rule between clusters.' }
	]
}
