...
export const zSwitchDoc: ComponentDocT = {
	...
	playground: {
		...
		controlNames: ['size', 'accent', 'is-checked', 'is-disabled', 'is-full-width'],
	...
	usageGuidance: [
		...
		'For a settings row, `is-full-width` plus a row layout puts the label on the left and the switch on the right, which is where the eye looks for it.',
		'A switch is not for toolbars — that is `z-toggle-button`, which reads as a pressed button rather than a setting.',
	...
	examples: [
		...
		defineInteractiveExample({
			...
			markup: `
				<z-switch id="previewSwitch" accent="dom" is-full-width>Show previews in the sidebar</z-switch>
		...
		defineInteractiveExample({
			...
			layout: ExampleLayout.stack,
			markup: `
				<z-switch id="notificationsSwitch" accent="dom" is-full-width is-checked>Email notifications</z-switch>
				<wired-column gap="xs" style="padding-left: 1rem">
				  <z-switch is-full-width size="sm" class="notificationChannel" is-checked>Mentions</z-switch>
				  <z-switch is-full-width size="sm" class="notificationChannel">Weekly digest</z-switch>
				</wired-column>
			`,
			script: `
	...
	related: [
		{ tag: 'z-checkbox', route: '/elements/actionables/z-checkbox', description: 'When the value is committed on submit rather than applied immediately.' },
		{ tag: 'z-toggle-button', route: '/elements/actionables/z-toggle-button', description: 'The toolbar equivalent — a button that stays pressed.' },
		{ tag: 'z-field', route: '/elements/actionables/z-field', description: 'Label and help text treatment.' }
	]
}
