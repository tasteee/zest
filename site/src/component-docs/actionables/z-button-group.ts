...
export const zButtonGroupDoc: ComponentDocT = {
	...
	usageGuidance: [
		...
		'Reach for `z-toggle-button-group` instead when one of the segments should stay visibly selected after the click.'
	],
	...
	examples: [
		...
		defineInteractiveExample({
			...
			description:
				'The group holds no state, so a "currently selected" segment is yours to manage — here by swapping `kind` on click. If this is all you need, `z-toggle-button-group` does it for you.',
	...
	related: [
		{ tag: 'z-button', route: '/elements/actionables/z-button', description: 'The children this group arranges.' },
		{ tag: 'z-toggle-button-group', route: '/elements/actionables/z-toggle-button-group', description: 'A segmented control that tracks selection.' },
		{ tag: 'z-toolbar', route: '/elements/actionables/z-toolbar', description: 'An action strip with roving tabindex.' }
	]
}
