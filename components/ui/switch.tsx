...
type SwitchVariantT = 'neutral' | 'purple' | 'pink'

type SwitchVariantOptionsT = {
	variant?: SwitchVariantT | null
}

const switchVariants = (options?: SwitchVariantOptionsT): string => {
	...
	return cn(
		...
		variant === 'neutral' && 'data-[state=checked]:bg-primary',
		variant === 'purple' && 'data-[state=checked]:bg-neon-purple',
		variant === 'pink' && 'data-[state=checked]:bg-neon-pink'
	)
}
...
function Switch({
	...
	variant,
	...props
}: React.ComponentProps<typeof SwitchPrimitive.Root> & SwitchVariantOptionsT) {
	return (
