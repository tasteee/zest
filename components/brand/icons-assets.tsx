...
const colorShowcaseEntries: ColorShowcaseEntryT[] = [
	...
	{ label: 'Success', colorClass: 'text-neon-purple' },
	{ label: 'Accent', colorClass: 'text-neon-pink' },
	{ label: 'Info', colorClass: 'text-neon-purple' },
	{ label: 'Warning', colorClass: 'text-neon-pink' },
...
export const IconsAssets = (props: IconsAssetsPropsT) => {
	...
	return (
		<section ref={sectionRef} id='icons' className='min-h-screen px-6 lg:px-12 py-24 border-t border-border'>
			<div className='max-w-7xl mx-auto'>
				{/* Section Header */}
				<div className='mb-20'>
					<div className='flex items-center gap-4 mb-6'>
						<span className='text-neon-pink text-xs font-semibold tracking-[0.2em] uppercase'>05</span>
				...
				{/* Standard Banner */}
				<div className='mb-16 border border-neon-purple/30 rounded-lg p-6 bg-neon-purple/5'>
					<div className='flex items-start gap-4'>
						<Phosphor.Checks size={24} weight='bold' className='text-neon-purple mt-0.5 shrink-0' />
						<div>
							<h3 className='text-primary font-semibold text-base mb-1'>Phosphor Icons — Official Standard</h3>
							<p className='text-muted-foreground text-sm leading-relaxed'>
								All icons in this product use <span className='text-neon-purple font-semibold'>@phosphor-icons/react</span>. Do not
								use lucide-react, heroicons, or custom inline SVGs. Phosphor provides 1,248+ icons in 6 weights: thin, light,
								regular, bold, fill, and duotone.
							</p>
							<code className='mt-3 block text-xs font-mono text-neon-pink bg-black/30 px-3 py-2 rounded'>
								{`import * as Phosphor from "@phosphor-icons/react";`}
							</code>
						</div>
					</div>
					...
					<div className='grid grid-cols-4 md:grid-cols-8 lg:grid-cols-10 gap-4'>
						{iconEntries.map((iconEntry) => {
							...
							return (
								...
								>
									<div className='text-foreground group-hover:text-neon-purple transition-colors'>
					...
					<div className='border border-border rounded-lg p-8'>
						<div className='flex flex-wrap gap-10'>
							{weightShowcaseEntries.map((weightEntry) => {
								return (
									<div key={weightEntry.label} className='flex flex-col items-center gap-3'>
										<Phosphor.Star size={32} weight={weightEntry.weight} className='text-foreground' />
										<span className='text-muted-foreground text-xs font-mono'>{weightEntry.label}</span>
									</div>
								)
							})}
						</div>
				...
				<div className='mb-24'>
					...
					<div className='border border-border rounded-lg p-8'>
						<span className='text-muted-foreground text-xs tracking-wider uppercase block mb-6'>Semantic Colors</span>
							...
							{colorShowcaseEntries.map((colorEntry) => {
								return (
									<div key={colorEntry.label} className={colorEntry.colorClass}>
										...
										<span className='text-xs mt-2 block'>{colorEntry.label}</span>
									</div>
								)
							})}
						</div>
				...
				<div className='mb-24'>
					...
					<div className='grid md:grid-cols-2 gap-6'>
						<div className='border border-border rounded-lg p-6'>
							<span className='text-neon-purple text-xs font-semibold tracking-wider uppercase block mb-4'>Do</span>
						...
						<div className='border border-border rounded-lg p-6'>
							<span className='text-neon-pink text-xs font-semibold tracking-wider uppercase block mb-4'>Don&apos;t</span>
				...
				<div className='mb-24'>
					...
					<div className='grid md:grid-cols-2 gap-8'>
						<div className='border border-border rounded-lg p-12 flex flex-col items-center justify-center'>
							...
							<span className='text-muted-foreground text-xs tracking-wider uppercase mt-8'>Primary Lockup</span>
						</div>

						<div className='border border-border rounded-lg p-12 flex flex-col items-center justify-center'>
							<div className='w-16 h-16 rounded-full bg-neon-purple mb-8' />
							<span className='text-muted-foreground text-xs tracking-wider uppercase'>Icon Mark</span>
						</div>
					</div>
						...
						<div className='flex flex-col lg:flex-row lg:items-center gap-8'>
							<div className='flex-1'>
								...
								<p className='text-foreground text-sm leading-relaxed'>
									Maintain minimum clear space equal to the height of the icon mark around all logo usage.
								</p>
							</div>
							<div className='flex items-center gap-4'>
								<span className='text-neon-pink text-xs font-semibold'>Min: 24px</span>
								<span className='text-neon-purple text-xs font-semibold'>Preferred: 48px</span>
							</div>
						</div>
					</div>
				</div>
							...
							{[4, 8, 12, 16, 24, 32, 48, 64].map((space) => {
								return (
									<div key={space} className='text-center'>
										<div
											className='bg-neon-purple/20 border border-neon-purple/50 mx-auto mb-2'
											style={{
												width: `${Math.min(space, 64)}px`,
												height: `${Math.min(space, 64)}px`
											}}
										/>
										<span className='text-foreground text-xs font-mono'>{space}px</span>
									</div>
								)
							})}
						</div>
						<p className='text-muted-foreground text-sm'>
							Use consistent spacing increments. Spacious layouts feel premium. Cramped layouts feel desperate.
						</p>
					</div>
				</div>
			</div>
