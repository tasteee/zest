...
export function ColorSystem({ onInView }: ColorSystemProps) {
	...
	return (
		<section ref={sectionRef} id='colors' className='min-h-screen px-6 lg:px-12 py-24 border-t border-border'>
			<div className='max-w-7xl mx-auto'>
				{/* Section Header */}
				<div className='mb-20'>
					<div className='flex items-center gap-4 mb-6'>
						<span className='text-neon-pink text-xs font-semibold tracking-[0.2em] uppercase'>01</span>
				...
				<div className='mb-24'>
					...
					<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
						{brandColors.map((color) => (
							...
							>
								<div className='h-32 w-full' style={{ backgroundColor: color.value }} />
								<div className='p-5'>
				...
				<div className='mb-24'>
					...
					<div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4'>
						{grayScale.map((gray) => (
							<div key={gray.name} className='group cursor-pointer' onClick={() => copyToClipboard(gray.value, gray.name)}>
								<div className={cn('h-20 w-full rounded-lg border border-border mb-3', gray.class)} />
						...
						<div>
							...
							<ul className='space-y-3 text-foreground text-sm'>
								<li className='flex items-start gap-3'>
									...
									<span>
										Never use <span className='text-muted-foreground line-through'>blue</span>. Seriously. Not even close.
									</span>
								</li>
								<li className='flex items-start gap-3'>
									<span className='text-neon-pink mt-1'>•</span>
