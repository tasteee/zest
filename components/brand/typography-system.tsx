...
export function TypographySystem({ onInView }: TypographySystemProps) {
	...
	return (
		<section ref={sectionRef} id='typography' className='min-h-screen px-6 lg:px-12 py-24 border-t border-border'>
			<div className='max-w-7xl mx-auto'>
				{/* Section Header */}
				<div className='mb-20'>
					<div className='flex items-center gap-4 mb-6'>
						<span className='text-neon-purple text-xs font-semibold tracking-[0.2em] uppercase'>02</span>
						<div className='h-px flex-1 max-w-16 bg-border' />
					</div>
					<h2 className='text-primary font-bold text-4xl md:text-5xl lg:text-6xl tracking-tight mb-6'>Typography</h2>
				...
				<div className='mb-24 border border-border rounded-lg p-8 lg:p-12'>
					<div className='flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-12'>
						<div>
							<span className='text-neon-pink text-xs font-semibold tracking-[0.2em] uppercase block mb-4'>Primary Typeface</span>
							<h3 className='text-primary font-bold text-6xl md:text-7xl lg:text-8xl tracking-tight'>DM Sans</h3>
						</div>
					...
					<p className='text-foreground text-lg leading-relaxed mt-4 max-w-3xl'>ABCDEFGHIJKLMNOPQRSTUVWXYZ</p>
					...
					<div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6'>
						{weights.map((weight) => (
							<div key={weight.name} className='border border-border rounded-lg p-6 text-center'>
								<span className='text-primary text-4xl block mb-4' style={{ fontWeight: weight.value }}>
									Aa
								</span>
						...
						{typeScale.map((type) => (
							...
							>
								...
								<div className='flex-1'>
									<p className={`text-primary ${type.class}`}>Fuck it, I guess.</p>
								</div>
				...
				<div className='border border-border rounded-lg p-8 lg:p-12'>
					...
					<div className='grid lg:grid-cols-2 gap-12'>
						<div className='space-y-6'>
							<h4 className='text-primary font-bold text-3xl leading-tight'>Headlines demand attention.</h4>
							<p className='text-foreground leading-relaxed'>
								Body text carries the message. It&apos;s readable, comfortable, and gets the job done without stealing the
								spotlight. Keep it gray, not white.
							</p>
