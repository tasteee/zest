...
export const ZNumberInput = c(
	(props) => {
		...
		const isInvalid = Boolean(props.invalid) || (rawValue !== '' && !isInRange)
		...
		return <host shadowDom><div class={fieldClass}>
			{props.hasStepperButtons && <button class="stepper" type="button" disabled={props.disabled || props.isReadonly || (min != null && (parsed ?? value ?? min) <= min)} aria-label="Decrease value" onclick={() => stepValue(-1)}>−</button>}
			<input
				type="text"
				inputmode="decimal"
				value={rawValue}
			...
			/>
			{props.hasStepperButtons && <button class="stepper" type="button" disabled={props.disabled || props.isReadonly || (max != null && (parsed ?? value ?? max) >= max)} aria-label="Increase value" onclick={() => stepValue(1)}>+</button>}
		</div></host>
	},
	{
		props: {
