...
const styles = css`
  ...
  /* A tint of the text colour rather than a named surface, so it sits one
	   step off whatever background the rail has in every theme. --muted is
	   the muted *text* colour here, not a fill — it would sit under the label
	   at well under 2:1. */
  .link:hover {
...
export const ZNavTree = c(
  (props) => {
    ...
    const [overrides, setOverrides] = useState<BranchOverridesT>(() =>
      readOverrides(storageKey),
    );
    const [filterText, setFilterText] = useState("");

    const items: NavNodeT[] = Array.isArray(props.items)
      ? (props.items as NavNodeT[])
      : [];
    ...
    const renderRow = (row: NavRowT) => {
      ...
      const rowStyle = isNested
        ? `margin-left: ${indent}; padding-left: 0.25rem`
        : undefined;
