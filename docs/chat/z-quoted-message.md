...
| `text` | string | — | preview text (clamped to 2 lines) |
| `value` | string | — | identifier passed back in `jump`'s `detail.value` |
| `tone` | `secondary` | primary | accent bar color |
| `no-jump` | boolean | — | disable the click-to-jump behavior (renders non-interactive) |
| `is-hidden` | boolean | — | hide |

## Slots

- _(default)_ — additional content appended after `text`.

## Events

| Event | `detail` | Description |
| --- | --- | --- |
| `jump` | `{ value }` | clicked (unless `no-jump` is set) |
