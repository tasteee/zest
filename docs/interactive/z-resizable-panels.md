...
```html
<z-resizable-panels direction="horizontal" auto-save-id="editor" style="height: 22rem">
...
A group sizes itself to `100%` of its parent on both axes. A horizontal
group still renders without one — block-level ancestors are as wide as the
page by default — but a vertical group has nothing to be `100%` *of* unless
something up the chain sets a real height, so it collapses to the height of
its panels' own content instead of the percentages you gave it. Set a height
on the group itself (as above), or on a sized ancestor, before switching
