...
A floating contextual menu with three variants, switched via `kind`. Use
...
<z-bubble-menu kind="image"></z-bubble-menu>
<z-bubble-menu kind="table-cell"></z-bubble-menu>
```

```js
const linkBubble = document.querySelector('z-bubble-menu[kind="link"]')
...
| --- | --- | --- | --- |
| `kind` | `link` `image` `table-cell` | `link` | which control set to render |
