...
<z-eyebrow tone="primary" full-width>Changelog</z-eyebrow>
```
...
| --- | --- | --- | --- |
| `tone` | `primary` `neutral` | secondary/pink | accent color |
| `label` | string | — | text (alternative to slotting children) |
| `has-rule` | boolean | — | show a trailing hairline rule |
| `full-width` | boolean | — | stretch the rule to fill the container instead of a fixed width |
